...
int main(int argc, char* argv[])
{
    ...
    while (true)
    {
        ...
        if (connected())
        {
            ...
            //if there is not two spaces in line
            if (control!=2)
            {error(400);continue;}
            ...
            //if there are more than one '?' in line
            if (control > 1)
            {error(400);continue;}
            ...
            {absolute_path[i]=0;}
            control = 0;
            for (int i=0,j=0;(i<n )&& (control != 2);i++)
            {
                if  (line[i]==' '||line[i]=='?')
                {control++;} 
                else if (control == 1)
                {
                    absolute_path[j] = line[i];
                    j++;
                }         
            }  
  ...
  /**************************************************************************/          
            ...
            {extension[i]=0;} 
            control = 0;
            for (int i=0,j=0;i<n && control != 3 ;i++)
            {
